...
namespace PlataformaEducacao.Bff.Api.Tests.Config
{
    public class TestAuthHandler : AuthenticationHandler<AuthenticationSchemeOptions>
    {
        ...
        protected override Task<AuthenticateResult> HandleAuthenticateAsync()
        {
            ...
            {
                return Task.FromResult(AuthenticateResult.Fail("Usuario nao fornecido"));
            }

            var userRole = role.ToString();
            var userId = Context.Request.Headers.TryGetValue("X-Test-UserId", out var requestedUserId)
