...
namespace PlataformaEducacao.GestaoAluno.Application.Services
{
    public class PagamentoMatriculaIntegrationHandler : BackgroundService
    {
        ...
        {
            _bus.SubscribeAsync<MatriculaPagamentoRecusadoIntegrationEvent>(
                "MatriculaRecusada", async request => await RecusarMatricula(request));

            _bus.SubscribeAsync<MatriculaPagamentoRealizadoIntegrationEvent>(
                "MatriculaConfirmada", async request => await FinalizarMatricula(request));
        }
        ...
        private async Task RecusarMatricula(MatriculaPagamentoRecusadoIntegrationEvent message)
        {
            ...
            var matricula = await alunoRepository.ObterMatriculaComAlunoPorId(message.MatriculaId, default)
            ...
            if (await alunoRepository.UnitOfWork.Commit() is false)
